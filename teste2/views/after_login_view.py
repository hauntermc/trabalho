...
from PyQt5.QtGui import QPixmap, QIcon, QPageSize, QPdfWriter, QPageLayout
    ...
    def print_pdf(self):
        ...
        if print_dialog.exec_() == QPrintDialog.Accepted:
            try:
                ...
                pdf_writer.setPageMargins(15, 15, 15, 15, QPageLayout.Millimeter)

                pdf_document = fitz.open(self.file_path)
                if pdf_document.page_count > 0:
                    pdf_writer.newPage()
                    pdf_writer.writeDocument(pdf_document)
                else:
                    QMessageBox.warning(self, 'Erro', 'O PDF está vazio ou não contém páginas.')
...
class AfterLoginScreen(QWidget):
   ...
   #
    def add_buttons_to_tab(self, layout, button_names):
        ...
        layout.setContentsMargins(0, 0, 0, 0)  # Remova as margens internas do layout
